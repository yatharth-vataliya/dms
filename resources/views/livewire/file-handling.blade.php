...
<div class="bg-blue-100 m-2 p-2 rounded">
    ...
    <div class="text-center rounded-full w-68 p-4 m-2 bg-white shadow text-lg text-teal-700 grid grid-cols-2 items-center items-baseline">
        <div>
            {{ $used_space }} MB used out of {{ auth()->user()->storage_limit }} GB
        </div>
        <div>
            <input type="text" name="search" id="search" wire:model="search" class="focus:outline-none focus:shadow-outline-green p-2 rounded bg-green-300 text-black focus:bg-white w-64 placeholder-green-700" placeholder="Search File Here">
        </div>
    </div>
    <div class="bg-white rounded p-4 grid grid-cols-4">
            ...
            <div class="p-4 m-2 bg-green-200 text-center hover:bg-green-300 hover:shadow-md rounded break-all">
                ...
                <div class="p-2 bg-gray-300 rounded-md">
                    <div class="grid grid-cols-4">
                        <a href="{{ asset('storage/'.$file->file_url) }}" download="{{ $file->file_name }}" class="hover:shadow-md rounded hover:bg-white">
